...
class GameSettings {
  ...
  static ChangeMap() {
    ...
    if (sel != undefined) div.removeChild(sel); //ako postoji izbriši
    ...
    sel.addEventListener("change", function () {
      ...
      let t =
        '<span class="emoji">&#128736;</span> Setup game: <b style="color: blue">x</b>';
...
function btnStart_click() {
  ...
  if (selMaps) GameSettings.hideElement(selMaps);
...
function btnStop_click() {
  ...
  if (selMaps) GameSettings.showElement(selMaps);
}

function showOutput_change(event) {
  if (this.checked) document.getElementById("taInfo").removeAttribute("hidden");
  else document.getElementById("taInfo").setAttribute("hidden", true);
